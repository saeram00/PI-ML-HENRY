...
def get_score_count(platform: str, scored: float, year: int) -> int:
    ...
    filtered_df = df.loc[(df["platform"] == platform) & (df["release_year"] == year)]

    # Count movies with score higher than "scored" param
    movie_count = filtered_df.loc[filtered_df["score_avg"] > scored].groupby("platform")["title"].count()[platform]
